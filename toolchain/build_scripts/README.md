...
```sh
docker run --user "$(id -u):$(id -g)" -v `pwd`:/toolchain/build_scripts -w /toolchain/build_scripts toolchain_test bash coralnpu_v2_toolchain_build.sh
```

Output toolchain artifacts will be exported to rv_multilib_out
