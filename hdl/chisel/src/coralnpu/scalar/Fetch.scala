...
class Fetch(p: Parameters) extends FetchUnit(p) {
  ...
  def Predecode(addr: UInt, op: UInt): (Bool, UInt) = {
    ...
    val immed =
      Cat(Fill(p.programCounterBits - 20, op(31)), op(19, 12), op(20), op(30, 21), 0.U(1.W))
  ...
  val preBranchTaken = (0 until p.instructionLanes)
    .map(i => io.inst.lanes(i).valid && preBranchTakens(i))
    .reduce(_ || _)
  ...
  aslice.io.in.bits  := MuxCase(
    instAligned1,
    (0 until p.instructionLanes).map(x =>
      reqs(x) -> Cat(io.branch(x).value(p.programCounterBits - 1, indexLsb), 0.U(indexLsb.W))
    ) ++
      Array(
  ...
  val fsela = Cat(
    (0 until p.instructionLanes).reverse.map(x =>
      (x until p.instructionLanes)
        .map(y =>
          (if (y == x) { fetchEn(y) }
           else { !fetchEn(y) })
        )
  ...
  val nxtInstAddrOffset =
    instAddr.map(x => x) ++ instAddr.map(x => x + (p.instructionLanes * instrBytes).U)
  val nxtInstAddr = (0 until p.instructionLanes).map(i =>
    (0 until (p.instructionLanes + 1))
      .map(j => MuxOR(fsel(j), nxtInstAddrOffset(j + i)))
      .reduce(_ | _)
  )
  ...
  val readFwd0 =
    readDataEn && readAddr(p.programCounterBits - 1, indexLsb) === instAligned0(
      p.programCounterBits - 1,
      indexLsb
    )
  val readFwd1 =
    readDataEn && readAddr(p.programCounterBits - 1, indexLsb) === instAligned1(
      p.programCounterBits - 1,
      indexLsb
    )
  ...
  def BranchMatchDe(valid: Bool, value: UInt): (Bool, UInt, Vec[UInt], Vec[UInt]) = {
    ...
    val match1 = l0valid(addr(p.instructionLanes - 1)(indexMsb, indexLsb)) &&
      addr(p.instructionLanes - 1)(tagMsb, tagLsb) === VecAt(
        l0tag,
        addr(p.instructionLanes - 1)(indexMsb, indexLsb)
      )
  ...
  def BranchMatchEx(branch: Vec[BranchTakenIO]): (Bool, UInt, Vec[UInt], Vec[UInt]) = {
    val valid = branch.map(x => x.valid).reduce(_ || _)

    val addrBase = MuxCase(
      branch(branch.length - 1).value,
      (0 until branch.length - 1).map(x => branch(x).valid -> branch(x).value)
    )
    ...
    val match1 = l0valid(addr(branch.length - 1)(indexMsb, indexLsb)) &&
      addr(branch.length - 1)(tagMsb, tagLsb) === VecAt(
        l0tag,
        addr(branch.length - 1)(indexMsb, indexLsb)
      )

    val vvalid = VecInit(
      (0 until branch.length).map(x =>
        Mux(addr(0)(wordSelMsb, wordSelLsb) <= ((lanes - 1) - x).U, match0, match1)
      )
    )
  ...
  def PredecodeDe(addr: UInt, op: UInt): (Bool, UInt) = {
    ...
    val immjal =
      Cat(Fill(p.programCounterBits - 20, op(31)), op(19, 12), op(20), op(30, 21), 0.U(1.W))
    val immbxx =
      Cat(Fill(p.programCounterBits - 12, op(31)), op(7), op(30, 25), op(11, 8), 0.U(1.W))
  ...
  val brchTakenDeOr = (0 until p.instructionLanes)
    .map(x => io.inst.lanes(x).ready && io.inst.lanes(x).valid && brchTakensDe(x))
    .reduce(_ || _)

  val brchTargetDe = MuxCase(
    brchTargetsDe(p.instructionLanes - 1),
    ...
    Cat(
      (0 until p.instructionLanes).reverse.map(x =>
        if (x == 0) { true.B }
          ...
          (0 until x).map(y => !brchTakensDe(y)).reduce(_ && _)
        }
      )
    )
  ...
  val brchFwd =
    Cat(
      (0 until p.instructionLanes).reverse.map(x =>
        brchTakensDe(x) && (if (x == 0) { true.B }
                            else { (0 until x).map(y => !brchTakensDe(y)).reduce(_ && _) })
      )
    )
  ...
  for (i <- 0 until p.instructionLanes) {
    ...
    instValid(i) := Mux(
      brchTakenEx,
      brchValidEx(i, 0) === ~0.U((i + 1).W),
      Mux(
        brchTakenDe,
        brchValidDe(i, 0) === ~0.U((i + 1).W),
        nxtInstValidUInt(i, 0) === ~0.U((i + 1).W)
      )
    ) && !io.iflush.valid

    instAddr(i) := Mux(brchTakenEx, brchAddrEx(i), Mux(brchTakenDe, brchAddrDe(i), nxtInstAddr(i)))
    ...
    val idx = Cat(
      instAddr(0)(indexLsb) =/= nxtInstAddr(i)(indexLsb),
      nxtInstAddr(i)(wordSelMsb, wordSelLsb)
    )
    instBits(i) := Mux(
      brchTakenEx,
      brchBitsEx(i),
      Mux(brchTakenDe, brchBitsDe(i), VecAt(nxtInstBits, idx))
    )
  }
