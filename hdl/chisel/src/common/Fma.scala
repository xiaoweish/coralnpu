...
object Fma {
  ...
  def FmaStage1(cmd: FmaCmd): FmaState1 = {
    ...
    state.exponent := Mux(raw_right_shift > 0.S, product_exponent, cmd.inc.exponent.zext)
  ...
  def FmaStage2(state1: FmaState1): FmaState2 = {
    ...
    val c_significand = Mux(state1.sub, -(state1.c_significand.zext), state1.c_significand.zext)
  ...
  def FmaStage3(state: FmaState2): Fp32 = {
    ...
    val reduced_significand =
      shifted_significand(shifted_significand.getWidth - 1, shifted_significand.getWidth - 26)
    // Perform rounding step, going to 26 bits
    // TODO(derekjchow): Rounding mode
    val rounded_significand = reduced_significand +& 1.U(1.W)
    // Get new mantissa
    val mantissa =
      Mux(rounded_significand(25), rounded_significand(24, 2), rounded_significand(23, 1))
