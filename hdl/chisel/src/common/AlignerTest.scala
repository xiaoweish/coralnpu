      ...
      for (i <- 0 until 1000) {
        ...
        for (i <- 0 until n) {
          ...
          dut.io
            ...
            .bits
            .a
            .poke(Random.between(0, Math.pow(2, dut.io.in(i).bits.a.getWidth)).toInt)
          dut.io
            .in(i)
            .bits
            .b
            .poke(Random.between(0, Math.pow(2, dut.io.in(i).bits.b.getWidth)).toInt)
          dut.io
            .in(i)
            .bits
            .c
            .poke(Random.between(0, Math.pow(2, dut.io.in(i).bits.c.getWidth)).toInt)
        }
        ...
        for (i <- 0 until n) {
          val valid_in = dut.io.in(i).valid.peek().litValue
          if (valid_in == 1) {
            assertResult(dut.io.in(i).bits.a.peek().litValue) {
              dut.io.out(outIdx).bits.a.peek().litValue
            }
            assertResult(dut.io.in(i).bits.b.peek().litValue) {
              dut.io.out(outIdx).bits.b.peek().litValue
            }
            assertResult(dut.io.in(i).bits.c.peek().litValue) {
              dut.io.out(outIdx).bits.c.peek().litValue
            }
            outIdx = outIdx + 1
          }
        }
